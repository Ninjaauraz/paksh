const {useState,useEffect,useMemo,useRef}=React;
    ...
    async function getLastSeen(){ const rows=await dbFetch("/profiles?select=last_seen_at&limit=1"); return (rows&&rows[0]&&rows[0].last_seen_at)||null; }
    async function bumpLastSeen(){ const uid=_uid(); if(!uid) return; return dbFetch("/profiles?id=eq."+uid, { method:"PATCH", headers:{ Prefer:"return=minimal" }, body:JSON.stringify({ last_seen_at:new Date().toISOString() }) }); }
    ...
    async function listFollowedTopics(){ return dbFetch("/follows_topic?select=topic"); }
    async function followTopic(topic){ return dbFetch("/follows_topic", { method:"POST", headers:{ Prefer:"resolution=merge-duplicates,return=minimal" },
      body:JSON.stringify({ user_id:_uid(), topic }) }); }
    async function unfollowTopic(topic){ return dbFetch("/follows_topic?topic=eq."+encodeURIComponent(topic), { method:"DELETE", headers:{ Prefer:"return=minimal" } }); }
    async function listFollowedStories(){ return dbFetch("/follows_story?select=story_id,topic,title,created_at&order=created_at.desc"); }
    async function followStory(story){ return dbFetch("/follows_story", { method:"POST", headers:{ Prefer:"resolution=merge-duplicates,return=minimal" },
      body:JSON.stringify({ user_id:_uid(), story_id:String(story.id), topic:story.topic||null, title:(story.headline||story.title||"").slice(0,300) }) }); }
    async function unfollowStory(storyId){ return dbFetch("/follows_story?story_id=eq."+encodeURIComponent(String(storyId)), { method:"DELETE", headers:{ Prefer:"return=minimal" } }); }
    ...
    const SECTION_HI = {
      india:"भारत", politics_policy:"राजनीति और नीति", economy:"अर्थव्यवस्था",
      finance_markets:"वित्त और बाज़ार", defence_security:"रक्षा और सुरक्षा", technology:"तकनीक",
      india_world:"भारत और विश्व", world:"विश्व", society:"समाज", health:"स्वास्थ्य",
      science_space:"विज्ञान और अंतरिक्ष", sports:"खेल", culture_entertainment:"संस्कृति और मनोरंजन",
    };
    ...
    const STR = {
      en: {
        ...
        search:"Search coverage…", tagline:"How India's media — Left, Centre and Right — covers each story.",
        ...
        framingTitle:"How each side is framing it", framingPending:"The side-by-side framing comparison appears once a full summary is generated for this story.", framingThin:"Not enough unique coverage to create a summary.",
      ...
      hi: {
        ...
        search:"कवरेज खोजें…", tagline:"भारत का मीडिया — वाम, केंद्र और दक्षिण — हर खबर को कैसे कवर करता है।",
        ...
        framingTitle:"हर पक्ष इसे कैसे पेश कर रहा है", framingPending:"इस ख़बर का पूरा सारांश तैयार होने पर पक्षों की तुलना यहाँ दिखाई देगी।", framingThin:"सारांश बनाने के लिए पर्याप्त स्वतंत्र कवरेज नहीं।",
      ...
      return (
        <footer className={`mt-12 border-t ${t.border} ${t.surface}`}>
          <div className="mx-auto max-w-[1280px] px-4 sm:px-10 py-9">
            ...
            {/* Corporate attribution, folded into the existing legal line rather than a new
                block - only the company name itself is a link (opens Redstocks' own site in a
                new tab; noopener+noreferrer since it's an external destination). */}
            <div className={`mt-7 border-t pt-5 ${t.border} mono text-[10.5px] uppercase tracking-wide ${t.tf}`}>
              {lang==="hi"
                ? <>© 2026 पक्ष · <a href="https://www.redstockstechnology.com/" target="_blank" rel="noopener noreferrer" className={`hover:${t.tp} hover:underline`}>Redstocks Technology</a> द्वारा निर्मित</>
                : <>© 2026 Paksh · Built by <a href="https://www.redstockstechnology.com/" target="_blank" rel="noopener noreferrer" className={`hover:${t.tp} hover:underline`}>Redstocks Technology</a></>
              }
            </div>
          </div>
        </footer>
          ...
          {items.map((s,i)=>{ const title=(lang==="hi"&&s.title_hi)?s.title_hi:s.title;
            return (
              <a key={s.id} href={"/storyline/"+encodeURIComponent(s.id)} onClick={e=>{ if(e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return; e.preventDefault(); goStoryline&&goStoryline(s.id); }} className={`block no-underline group cursor-pointer py-3 ${i<items.length-1||all.length>items.length?"border-b":""} ${t.border}`}>
                ...
                <div className={`mt-1 mono text-[10px] ${t.tf} ${lang==="hi"?"deva":""}`}><span aria-hidden="true" className={t.dev}>◇</span> {s.n_updates??s.n_events} {lang==="hi"?"अपडेट":"updates"}</div>
              </a>
            );
          })}
      ...
      const row=(s,big)=>{ const title=(lang==="hi"&&s.title_hi)?s.title_hi:s.title;
        ...
        return (
          <a key={s.id} href={"/storyline/"+encodeURIComponent(s.id)} onClick={e=>{ if(e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return; e.preventDefault(); goStoryline&&goStoryline(s.id); }} className={`block no-underline group cursor-pointer`}>
            ...
            <div className={`mt-1.5 mono text-[10.5px] ${t.tf} ${lang==="hi"?"deva":""}`}>{s.n_updates??s.n_events} {lang==="hi"?"अपडेट":"updates"}</div>
          </a>
        );
      };
      return (
    ...
    function HomeView({ cards, gapLeft, gapRight, topics, counts, stats, t, lang, open, goTopic, go, auth, lens, interests, interestSectionIds, openHelp, storylines, goStoryline, goStorylines }) {
      ...
      return (
        <div className="mx-auto max-w-[1280px]">
          ...
          <h1 className="sr-only">{lang==="hi"?"पक्ष: समाचार, संदर्भ के साथ।":"Paksh: News, with context."}</h1>
    ...
    function StoryPage({ story, t, lang, go, openTopic, related=[], open, a11y, auth, goStoryline }) {
      ...
      return (
        <div className="mx-auto max-w-[1000px] px-4 sm:px-8 py-6">
          ...
          {/* STORYLINE — how this saga developed across days. Gated on the SAME visible-entry
              count StorylineTimeline itself computes (genuine updates, plus the current story
              even if it was the one collapsed) - collapse safety: this story's own page is never
              affected by collapsing, only whether OTHER entries are worth cross-linking here. */}
          {story.storyline && (()=>{ const _visible=(story.storyline.events||[]).filter(ev=>ev.is_update!==false || String(ev.id)===String(story.id));
            ...
            <div className="mx-auto mt-10 max-w-[840px]">
              <div className="mb-1 flex items-baseline justify-between gap-3 pb-2" style={{borderBottom:`1px solid ${t.ink}`}}>
                ...
                <button onClick={()=>goStoryline&&goStoryline(story.storyline.id)} className={`mono text-[10.5px] ${t.tf} hover:${t.tp} ${lang==="hi"?"deva":""}`}>{_visible.length} {lang==="hi"?"अपडेट · पूरी कड़ी →":"updates · full storyline →"}</button>
              </div>
          ...
          <div className="mt-10">
            <div className="mb-4 flex items-baseline justify-between gap-3">
              {/* Phase 40D-B: exactly one lean covered -> "how sources are covering it"
                  (synthesis), not "how each side is framing it" (a comparison this
                  story structurally can't show, since there's only one side present).
                  2/3-lean stories are completely unchanged. */}
              <h2 className={`eyebrow ${t.tp} ${lang==="hi"?"deva":""}`} style={{letterSpacing:lang==="hi"?0:".14em"}}>{sides.length===1 ? STR[lang].framingTitleSingle : STR[lang].framingTitle}</h2>
          ...
          <div className="mx-auto mt-10 max-w-[840px]" id="arts" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
            ...
            {/* International / unrated outlets carry no voting lean, so say so when they appear
                rather than leave them looking like an omission (the only note kept here). */}
            {(counts.international>0||counts.unrated>0) && <p className={`mb-3 text-[11px] leading-relaxed ${t.tf} ${isHi(lang)}`}>{STR[lang].nonVoting}</p>}
    ...
    function TopicsHub({ topics, counts, t, lang, goTopic, cards }) {
      ...
      return (
        <div className="mx-auto max-w-[1000px] px-4 sm:px-8 py-10">
          <h1 className={`headline pk-text-display ${t.tp} ${readCls(lang)}`} style={{letterSpacing:lang==="hi"?0:"-0.018em"}}>{lang==="hi"?"सभी विषय":"All Topics"}</h1>
    ...
    function TopicPage({ topic, items, t, lang, open, go, auth, followingTopic, onToggleFollowTopic }) {
      ...
      return (
        <div className="mx-auto max-w-[1280px] px-4 sm:px-10 py-10">
          {/* Regression fix: /topics now lands on the new editorial-sections hub, not this
              raw-topic list, so this page's own parent hub is /all-topics (same TopicsHub
              component, still fully intact) - not /topics. */}
          <a href="/all-topics" onClick={e=>{ e.preventDefault(); go("all-topics"); }} className={`mb-4 inline-flex items-center gap-1.5 eyebrow ${t.ts} hover:${t.tp} ${isHi(lang)}`} style={{letterSpacing:lang==="hi"?0:".1em"}}><ArrowLeft size={14}/> {lang==="hi"?"सभी विषय":"All Topics"}</a>
          <div className="flex items-center justify-between gap-3 pb-3" style={{borderBottom:`2px solid ${t.ink}`}}>
            ...
            <div className="flex shrink-0 items-center gap-3">
              ...
              {authOn() && onToggleFollowTopic && <FollowButton on={!!followingTopic} onToggle={()=>onToggleFollowTopic(topic)}
                labelOn={lang==="hi"?"फ़ॉलो हो रहा है":"Following"} labelOff={lang==="hi"?"+ फ़ॉलो टॉपिक":"+ Follow topic"} t={t} lang={lang} />}
            </div>
          </div>
          {!items.length ? (
            <div className={`py-24 text-center ${t.tf} ${isHi(lang)}`}>{STR[lang].noStories}</div>
    ...
    function SectionsHub({ sections, cardById, t, lang, goSection, go }) {
      const LEAD_N=4;
      const lead=sections.slice(0,LEAD_N), rest=sections.slice(LEAD_N);
      // Regression fix: this hub replaced the raw-topic list as the "Sections" nav
      // destination, which quietly made /topic/<name> (and its Follow Topic button)
      // unreachable except from a story's own topic eyebrow link. Restored here as an
      // explicit, always-visible secondary link to /all-topics (the same TopicsHub
      // component, untouched, just no longer the primary nav destination) rather than
      // inventing a new follow affordance on the editorial sections themselves.
      const allTopicsLink = (
        <div className="mt-10 pt-6 text-center" style={{borderTop:`1px solid ${t.line}`}}>
          <a href="/all-topics" onClick={e=>{ e.preventDefault(); go&&go("all-topics"); }} className={`eyebrow ${t.ts} hover:${t.tp} ${isHi(lang)}`} style={{letterSpacing:lang==="hi"?0:".1em"}}>
            {lang==="hi"?"कच्चे विषय के अनुसार ब्राउज़ करें और फ़ॉलो करें →":"Browse & follow by raw topic →"}
          </a>
        </div>
      );
      if(!sections.length){
            ...
            {lead.map(s=>{ const lc=cardById[s.lead_id];
              return (
                <a key={s.key} href={"/section/"+s.slug} onClick={e=>{ e.preventDefault(); goSection(s.slug); }} className="block text-left">
                  <div className="flex items-baseline justify-between gap-3">
                    <span className={`headline text-[24px] sm:text-[28px] ${t.tp} ${readCls(lang)}`} style={{letterSpacing:lang==="hi"?0:"-0.014em"}}>{lang==="hi"?(SECTION_HI[s.key]||s.label):s.label}</span>
                    <span className={`mono text-[11px] shrink-0 ${t.tf}`}>{(s.all_story_ids||s.story_ids||[]).length}</span>
                  </div>
            ...
            <div className="mt-8 columns-2 lg:columns-3" style={{columnGap:"2.25rem"}}>
              {rest.map(s=>(
                <a key={s.key} href={"/section/"+s.slug} onClick={e=>{ e.preventDefault(); goSection(s.slug); }} className={`mb-0 flex w-full items-baseline justify-between gap-2 border-b py-2.5 text-left ${t.border}`} style={{breakInside:"avoid"}}>
                  <span className={`text-[14px] ${t.ts} hover:${t.tp} ${readCls(lang)}`}>{lang==="hi"?(SECTION_HI[s.key]||s.label):s.label}</span>
                  <span className={`mono text-[10.5px] shrink-0 ${t.tf}`}>{(s.all_story_ids||s.story_ids||[]).length}</span>
                </a>
              ))}
            </div>
          )}
    ...
    // already uses (SectionCard, BriefRow, BiasPill). Two-layer model (regression fix): `ranked`
    // (section_rank.py's top-N, diversified, seniority-ordered list - never re-sorted client-
    // side) drives ONLY the lead + secondary editorial area, exactly as before; `library` is the
    // section's FULL classified pool (uncapped) and is what "Show more" actually paginates
    // through, same completeness TopicPage's own /topic/<name> pages already give readers. The
    // handful of stories already shown in the lead/secondary area are filtered out of the
    // library list so they aren't shown twice.
    function SectionPage({ slug, spec, ranked, library, t, lang, open, go, followingSection, onToggleFollowSection }) {
      const label = spec ? (lang==="hi"?(SECTION_HI[spec.key]||spec.label):spec.label) : slug;
      const [visible,setVisible]=useState(20);
      const PAGE=20;
      const lead=ranked[0], section=ranked.slice(1,5);
      const shownIds=new Set(ranked.slice(0,5).map(c=>c.id));
      const restAll=library.filter(c=>!shownIds.has(c.id));
      const rest=restAll.slice(0,visible);
      const more=restAll.length-visible;
      const totalCount=library.length||ranked.length;
      return (
        <div className="mx-auto max-w-[1280px] px-4 sm:px-10 py-10">
          ...
          <div className="flex items-center justify-between gap-3 pb-3" style={{borderBottom:`2px solid ${t.ink}`}}>
            <h1 className={`headline pk-text-display ${t.tp} ${readCls(lang)}`} style={{letterSpacing:lang==="hi"?0:"-0.018em"}}>{label}</h1>
            <div className="flex shrink-0 items-center gap-3">
              <span className={`mono text-[11px] ${t.tf}`}>{totalCount}</span>
              {/* 2026-09-25 section IA: reuses the exact Follow mechanism TopicPage already has
                  (same follows_topic table via toggleFollowTopic) - see onToggleFollowSection at
                  its call site for the "section:" key prefix that keeps the two kinds apart. */}
              {authOn() && onToggleFollowSection && <FollowButton on={!!followingSection} onToggle={onToggleFollowSection}
                labelOn={lang==="hi"?"फ़ॉलो हो रहा है":"Following"} labelOff={lang==="hi"?"+ फ़ॉलो सेक्शन":"+ Follow section"} t={t} lang={lang} />}
            </div>
          </div>
          {!ranked.length ? (
            <div className={`py-24 text-center ${t.tf} ${isHi(lang)}`}>{STR[lang].noStories}</div>
    ...
    function PrivacyPage({ t, lang, consent, setConsent, adsConsent, setAdsConsent, go }) {
      ...
      const P = lang==="hi" ? {
        ...
        c2H:"विज्ञापन (Google)", c2:"जब तक आप अनुमति न दें, बंद रहता है। अनुमति देने पर Google विज्ञापन दिखाने व नापने के लिए, दूसरी साइटों पर भी, कुकी का उपयोग कर सकता है। आज तक कोई विज्ञापन नेटवर्क कॉन्फ़िगर नहीं है, इसलिए अभी कुछ भी लोड नहीं होता।",
        c3H:"खाता (वैकल्पिक)", c3:"खाता बनाना ज़रूरी नहीं। यदि आप बनाते हैं, तो सिर्फ़ साइन-इन कोड भेजने के लिए आपका ईमेल, और आपकी अनुमति से आपकी 'रीडिंग लेंस' (आप क्या पढ़ते हैं) व 'सहेजी खबरें' रखी जाती हैं। यह निजी है, बेचा नहीं जाता, और यह नहीं बदलता कि आपको कौन-सी खबरें दिखें।",
        ...
        note2:"डेटा के बारे में सवाल?", note2Link:"संपर्क करें"
      } : {
        ...
        c2H:"Advertising (Google)", c2:"Off unless you allow it. If you do, Google may use cookies to show and measure ads, including across other sites. No ad network is configured today, so nothing loads either way yet.",
        c3H:"Account (optional)", c3:"An account is never required to read Paksh. If you create one, we keep your email (only to send you a sign-in code) and, with your consent, which stories you open (Reading Lens) and any stories you save. It is private to you, never sold, and never used to change which stories you're shown.",
        ...
        note2:"Questions about your data?", note2Link:"Contact us"
      };
        ...
        <div className="mx-auto max-w-[1180px] px-4 sm:px-8 py-10">
          ...
          <div className="mt-7 grid lg:grid-cols-[1.6fr_1fr]">
            ...
            <div className="mt-7 lg:mt-0 lg:pl-8 space-y-4">
              ...
              <p className={`text-[11.5px] ${t.tf} ${isHi(lang)}`} style={{lineHeight:1.5}}>{P.note2} <a href="/contact" onClick={e=>{ e.preventDefault(); go&&go("contact"); }} className={`font-semibold ${t.ts} hover:${t.tp}`}>{P.note2Link}</a>.</p>
            </div>
          </div>
          {/* full legal policy */}
          <div className="mt-10 max-w-3xl">
          <h2 className={`headline text-[20px] ${t.tp} serif`}>Privacy Policy</h2>
          <p className={`mb-1 mt-2 text-[13px] ${t.tf}`}>Last updated: 25 September 2026 · Operated by Redstocks Technology LLP</p>
    ...
    function LoginPage({ t, lang, go, onAuthed }) {
      ...
      const L = lang==="hi" ? {
        ...
        checkEmail:"अपना ईमेल देखें", sentPre:"हमने ", sentPost:" पर एक 8 अंकों का कोड भेजा है।",
        codeL:"अपना 8 अंकों का साइन-इन कोड डालें", codeP:"8 अंकों का कोड", verifyBtn:"साइन इन करें", verifying:"जाँच हो रही है…",
        change:"ईमेल बदलें", resend:"कोड फिर भेजें", resent:"नया कोड भेज दिया।", resendIn:"फिर भेजें",
        useCode:"पासवर्ड के बजाय ईमेल कोड इस्तेमाल करें", usePw:"इसके बजाय पासवर्ड इस्तेमाल करें",
        pwSignupPrompt:"पक्ष में नए हैं? इसके बजाय पासवर्ड खाता बनाएँ", pwSigninPrompt:"पहले से पासवर्ड खाता है? इसके बजाय साइन इन करें",
        ...
        checkEmail:"Check your email", sentPre:"We sent an 8-digit code to ", sentPost:".",
        codeL:"Enter your 8-digit verification code", codeP:"8-digit code", verifyBtn:"Sign in", verifying:"Checking…",
        change:"Change email", resend:"Resend code", resent:"New code sent.", resendIn:"Resend in",
        useCode:"Use an email code instead", usePw:"Use a password instead",
        pwSignupPrompt:"New here? Create a password account instead", pwSigninPrompt:"Already have a password account? Sign in instead",
          ...
          : method==="password" ? (
            <form onSubmit={pwSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={()=>{setMode(mode==="signup"?"signin":"signup");setErr("");setNote("");}} className={link}>{mode==="signup"?L.pwSigninPrompt:L.pwSignupPrompt}</button>
              </div>
            </form>
          ) : step==="email" ? (
            <form onSubmit={send} className="space-y-4">
              ...
              <p className={`text-[12.5px] leading-[1.5] ${t.tf} ${readCls(lang)}`}>{L.otpHelp}</p>
              <button type="button" onClick={()=>{setMethod("password");setErr("");setNote("");}} className={link}>{L.usePw}</button>
            </form>
          ) : (
            <form onSubmit={verify} className="space-y-4">
              <div>
                <h2 className={`headline text-[17px] ${t.tp} ${readCls(lang)}`}>{L.checkEmail}</h2>
                <p className={`mt-1 text-[13px] ${t.ts} ${isHi(lang)}`}>{L.sentPre}<span className="font-semibold">{email}</span>{L.sentPost}</p>
              </div>
    ...
    function FollowButton({ on, onToggle, labelOn, labelOff, t, lang }) {
      return <button type="button" onClick={onToggle} aria-pressed={on?"true":"false"}
        className={`inline-flex items-center gap-1.5 border px-3 py-1.5 text-[12px] font-semibold ${on?`${t.cta} ${t.ctaT} border-transparent`:`${t.border} ${t.ts} hover:${t.tp}`} ${lang==="hi"?"deva":""}`}>
    ...
    function MyPakshPage({ t, lang, auth, go, open, savedRows, followedTopics, followedStoryRows, onToggleFollowTopic, onToggleFollowStory, cards }) {
      const [readingRows,setReadingRows]=useState(null);
      useEffect(()=>{ if(!auth) return; listReading(30).then(r=>setReadingRows(r||[])).catch(()=>setReadingRows([])); },[auth]);
      ...
      return (
        <PageWrap>
          <h1 className={`headline pk-text-display ${t.tp} ${readCls(lang)}`} style={{letterSpacing:lang==="hi"?0:"-0.018em"}}>{L.title}</h1>
          {loading ? <div className={`mt-8 py-10 text-center text-[13px] ${t.tf}`}>…</div>
          : !hasAnything ? (
            <div className={`mt-8 border border-dashed p-12 text-center ${t.border}`}>
              <div className={`text-[16px] font-semibold ${t.tp} ${isHi(lang)}`}>{L.firstTimeH}</div>
              <p className={`mx-auto mt-1.5 max-w-[44ch] text-[13.5px] ${t.tf} ${readCls(lang)}`}>{L.firstTimeB}</p>
              <button onClick={()=>go("home")} className={`mt-5 border px-4 py-2 eyebrow ${t.border} ${t.ts} hover:${t.tp} ${lang==="hi"?"deva":""}`} style={{letterSpacing:lang==="hi"?0:".08em"}}>{L.browse}</button>
            </div>
          ) : (
            <div className="mt-8 space-y-10">
              ...
              {sywlh.length>0 && (
                <div>
                  <SectionTitle t={t} lang={lang}>{L.sywlh}</SectionTitle>
              ...
              {(followedTopicList.length>0 || followedSectionList.length>0 || followedStories.length>0) && (
                <div>
                  <SectionTitle t={t} lang={lang}>{L.following}</SectionTitle>
                  {(followedTopicList.length>0 || followedSectionList.length>0) && (
                    <div className="flex flex-wrap gap-2">
                      {followedSectionList.map(key=>{ const enLabel=(SECTION_ORDER.find(([k])=>k===key)||[])[1]||key;
                        const slug=key.replace(/_/g,"-");
                        return (
                        <span key={"section:"+key} className={`inline-flex items-center gap-2 border px-3 py-1.5 text-[12px] font-semibold ${t.border} ${t.ts}`}>
                          <button onClick={()=>go("section/"+encodeURIComponent(slug))} className={`hover:${t.tp} ${lang==="hi"?"deva":""}`}>{lang==="hi"?(SECTION_HI[key]||enLabel):enLabel}</button>
                          <button onClick={()=>onToggleFollowTopic("section:"+key)} aria-label={L.unfollow} className={`hover:${t.blind}`}>×</button>
                        </span>
                        ); })}
                      {followedTopicList.map(tp=>(
                        <span key={tp} className={`inline-flex items-center gap-2 border px-3 py-1.5 text-[12px] font-semibold ${t.border} ${t.ts}`}>
                          <button onClick={()=>go("topic/"+encodeURIComponent(tp))} className={`hover:${t.tp} ${lang==="hi"?"deva":""}`}>{lang==="hi"?(TOPIC_HI[tp]||tp):tp}</button>
                          <button onClick={()=>onToggleFollowTopic(tp)} aria-label={L.unfollow} className={`hover:${t.blind}`}>×</button>
                        </span>
                      ))}
                    </div>
                  )}
                  {followedStories.length>0 && (
                    <div className="mt-3">
                      {followedStories.slice(0,8).map((r)=>(
                        <div key={r.story_id} className={`flex w-full items-center justify-between gap-3 border-b py-3 ${t.border}`}>
                          <button onClick={()=>open(r.story_id)} className={`min-w-0 flex-1 truncate headline text-[14.5px] text-left ${t.tp} ${readCls(lang)}`}>{r.title||r.story_id}</button>
                          <button onClick={()=>onToggleFollowStory({id:r.story_id})} className={`shrink-0 mono text-[10px] uppercase ${t.tf} hover:${t.blind}`}>{L.unfollow}</button>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {saved.length>0 && (
                <div>
                  <SectionTitle t={t} lang={lang} right={<button onClick={()=>go("saved")} className={`text-[12px] font-semibold ${t.tf} hover:${t.tp} ${lang==="hi"?"deva":""}`}>{L.seeAllSaved}</button>}>{L.saved}</SectionTitle>
                  <div>
                    {saved.map((r)=>(
                      <button key={r.story_id} onClick={()=>open(r.story_id)} className={`flex w-full items-center justify-between gap-3 border-b py-3 text-left ${t.border}`}>
                        <span className={`min-w-0 flex-1 truncate headline text-[14.5px] ${t.tp} ${readCls(lang)}`}>{r.title||r.story_id}</span>
                      </button>
                    ))}
                  </div>
              ...
              <div className="pt-2">
                <button onClick={()=>go("lens")} className={`text-[12.5px] font-semibold ${t.tf} hover:${t.tp} ${lang==="hi"?"deva":""}`}>{L.openLens}</button>
              </div>
            </div>
          )}
        </PageWrap>
    ...
    function StorylinePage({ id, lean, t, lang, open, go }) {
      ...
      return (
        <div className="mx-auto max-w-[840px] px-4 sm:px-8 py-10">
          ...
          <div className={`mt-2 mono text-[11px] ${t.tf} ${lang==="hi"?"deva":""}`}>{storyline.n_updates??storyline.n_events} {L.updates} · {absDate(storyline.start,lang)} → {absDate(storyline.end,lang)}</div>
    ...
    function parsePath(){
      ...
      if(seg.length===1 && ["blindspot","topics","all-topics","sources","about","contact","privacy","support","login","settings","account","saved","lens","storylines","my-paksh"].includes(seg[0])) return {view:seg[0]};
      return {view:"404"};
    }
    ...
    const SECTION_ORDER = [
      ["india","India"], ["politics_policy","Politics & Policy"], ["economy","Economy"],
      ["finance_markets","Finance & Markets"], ["defence_security","Defence & Security"],
      ["technology","Technology"], ["india_world","India & World"], ["world","World"],
    ...
    function Onboarding({ t, lang, setLang, onDone, interests, onToggleInterest }) {
      ...
      const steps = lang==="hi" ? [
        ...
        {k:"कवरेज गैप", b:"जब एक पक्ष कवर करे और दूसरा न करे, पक्ष उसे चिह्नित करता है।"},
        {k:"झुकाव प्रकाशन का", b:"हर आउटलेट का झुकाव संपादक तय करते हैं, कोई एल्गोरिद्म नहीं।"},
        {k:"द्विभाषी", b:"हर खबर अंग्रेज़ी और हिंदी में। कभी भी भाषा बदलें।"},
      ] : [
        {k:"The bias bar", b:"Counts how many outlets lean Left, Centre or Right — one publisher, one vote."},
        {k:"Coverage gaps", b:"Flagged when one side covers a story and the other doesn't."},
        {k:"Lean is the publisher's", b:"Set by editors for each outlet, never guessed by an algorithm."},
        {k:"Bilingual", b:"Every story in English and Hindi. Switch anytime."},
      ];
      const L = lang==="hi"
          ...
          <div className={`pk-sheet w-full max-w-[460px] border-t sm:border ${t.surface} ${t.border}`} style={{boxShadow:"0 -8px 40px rgba(0,0,0,0.30)"}}>
            ...
            {step===0 ? (
              ...
              <div className="px-5 pb-5 pt-3">
                <h2 className={`headline mt-1 text-[21px] ${t.tp} ${readCls(lang)}`}>{L.interestsH}</h2>
                <p className={`mt-1.5 text-[13.5px] ${t.ts} ${readCls(lang)}`} style={{lineHeight:lang==="hi"?1.75:1.6}}>{L.interestsB}</p>
                <div className="mt-4 flex flex-wrap gap-2">
                  {SECTION_ORDER.map(([k,enLabel])=>{ const on=(interests||[]).includes(k);
                    return <button key={k} onClick={()=>onToggleInterest(k)} className={`border px-3 py-1.5 text-[12.5px] font-semibold ${on?`${t.cta} ${t.ctaT} border-transparent`:`${t.border} ${t.ts} hover:${t.tp}`} ${lang==="hi"?"deva":""}`}>{lang==="hi"?(SECTION_HI[k]||enLabel):enLabel}</button>; })}
                </div>
    ...
    function PakshApp() {
      ...
      const refreshFollows=()=>{
        ...
        listFollowedStories().then(rows=>{ rows=rows||[]; setFollowedStoryRows(rows); setFollowedStories(new Set(rows.map(r=>String(r.story_id)))); }).catch(()=>{ setFollowedStoryRows([]); });
      };
      ...
      // ids, never article content (see export_static.py) - the frontend resolves each id
      // against the SAME baseCards already loaded for topics/search. Two layers, matching
      // section_rank.select_all_sections_full(): `story_ids` is the top-N ranked/diversified
      // list (the lead + secondary editorial area - rendered in that exact order, never
      // re-sorted client-side), `all_story_ids` is the FULL classified library (uncapped,
      // newest-first) - a section's "show more" must browse this, not the top-N ranking
      // (regression fix: the top-N ranking was previously mistaken for the entire section,
      // capping every section's visible library at ~20 stories regardless of how many
      // actually qualified).
      ...
      const sectionByKey={}; sectionsOrdered.forEach(s=>{ sectionByKey[s.key]=s; });
      const itemsForSection=(slug)=>{ const s=sectionBySlug[slug];
        if(!s) return {ranked:[], library:[]};
        const ranked=(s.story_ids||[]).map(id=>cardById[id]).filter(Boolean);
        const library=(s.all_story_ids&&s.all_story_ids.length?s.all_story_ids:s.story_ids||[]).map(id=>cardById[id]).filter(Boolean);
        return {ranked, library};
      };
      ...
      const interestSectionIds = useMemo(()=>{
        const set=new Set();
        (interests||[]).forEach(k=>{ const s=sectionByKey[k];
          if(!s) return;
      ...
      useEffect(()=>{
        ...
        else if(route.view==="all-topics") title = suffix(lang==="hi"?"सभी विषय":"All Topics");
        else if(route.view==="section") { const sp=(data.sections||[]).find(s=>s.slug===route.slug); title = suffix(sp ? (lang==="hi"?(SECTION_HI[sp.key]||sp.label):sp.label) : ui("sections",lang)); }
      ...
      return (
        ...
        <div className={`min-h-screen font-sans ${t.bg} ${t.tp}`}>
          ...
          <main id="main" className="pb-24 md:pb-10">
            ...
            : route.view==="my-paksh" ? <MyPakshPage t={t} lang={lang} auth={auth} go={go} open={open} savedRows={savedRows} followedTopics={followedTopics} followedStoryRows={followedStoryRows} onToggleFollowTopic={toggleFollowTopic} onToggleFollowStory={toggleFollowStory} cards={baseCards} />
            ...
            : route.view==="topics" ? <SectionsHub sections={sectionsOrdered} cardById={cardById} t={t} lang={lang} goSection={goSection} go={go} />
            : route.view==="all-topics" ? <TopicsHub topics={topicsOrdered} counts={countsByTopic} cards={baseCards} t={t} lang={lang} goTopic={goTopic} />
            : route.view==="section" ? (()=>{ const _sec=itemsForSection(route.slug); const _spec=sectionBySlug[route.slug]; const _skey=_spec&&_spec.key;
                return <SectionPage slug={route.slug} spec={_spec} ranked={_sec.ranked} library={_sec.library} t={t} lang={lang} open={open} go={go}
            ...
            : route.view==="privacy" ? <PrivacyPage t={t} lang={lang} consent={consent} setConsent={setConsentChoice} adsConsent={adsConsent} setAdsConsent={setAdsChoice} go={go} />
            ...
            : (!homeCards.length ? <PageWrap><div className={`py-28 text-center ${t.tf} ${isHi(lang)}`}>{STR[lang].noStories}</div></PageWrap>
               : <HomeView cards={homeCards} gapLeft={gapL} gapRight={gapR} topics={topicsOrdered} counts={countsByTopic} stats={stats} t={t} lang={lang} open={open} goTopic={goTopic} go={go} auth={auth} lens={lensStats} interests={interests} interestSectionIds={interestSectionIds} openHelp={()=>go("about")} storylines={data.storylines} goStoryline={goStoryline} goStorylines={goStorylines} />)}
            </div>
          </main>
