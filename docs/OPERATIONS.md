...
| `BACKUP FAILED` | disk full / DB locked | Free disk (C: is ~94% full), then `py backup_db.py --keep 5` |

## Known gaps (deliberately not built)

- **Backups are on the same disk as the database.** `backups/` protects against corruption and bad
  migrations, not disk loss. Copy the newest `backups/paksh_backup_*.db` off this machine regularly
  (external drive or cloud bucket - Sameer's choice). About 10 GB of old unmanaged
  `paksh.db.bak*` / `paksh.db.pre_phase*` files sit in the project root and could be archived
  elsewhere to free space; nothing deletes them automatically.
